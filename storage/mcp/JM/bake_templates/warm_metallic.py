...
for obj in bpy.context.selected_objects:
    if obj.type == 'MESH':
        ...
        if not has_materials:
            mat = bpy.data.materials.new(name="WarmMetal_" + model_name)
            mat.use_nodes = True
            ...
            color_ramp.color_ramp.elements[0].color = (0.3, 0.15, 0.05, 1.0)
            color_ramp.color_ramp.elements[1].position = 1.0
            color_ramp.color_ramp.elements[1].color = (0.95, 0.75, 0.3, 1.0)
            elem = color_ramp.color_ramp.elements.new(0.5)
            elem.color = (0.8, 0.5, 0.15, 1.0)

            mix_rgb = nodes.new('ShaderNodeMix')
            mix_rgb.location = (0, 200)
            mix_rgb.data_type = 'RGBA'
            mix_rgb.inputs[0].default_value = 0.3
            mix_rgb.inputs[6].default_value = (0.7, 0.4, 0.1, 1.0)
            mix_rgb.inputs[7].default_value = (0.9, 0.7, 0.2, 1.0)
            ...
            print(f"Warm metallic material applied: {{obj.name}}")
...
for obj in bpy.context.selected_objects:
    if obj.type == 'MESH' and obj.data.materials:
        ...
        principled2.inputs['Metallic'].default_value = 0.7
