...
export function summarizeTrigger(cfg: TriggerConfig): string {
  switch (cfg.frequency) {
    case "hourly": {
      ...
      return t("Hourly · :{min}", { min });
    }
    case "daily":
      return t("Daily {time}", { time: cfg.time });
    case "weekdays":
      return t("Weekdays {time}", { time: cfg.time });
    case "weekly":
      return `${formatDayList(cfg.daysOfWeek)} ${cfg.time}`;
    case "custom":
      return t("Custom cron");
  }
}
...
export function describeTrigger(cfg: TriggerConfig): string {
  ...
  switch (cfg.frequency) {
    case "hourly": {
      ...
      return t("Runs every hour at :{min}", { min: min.toString().padStart(2, "0") });
    }
    case "daily":
      return t("Runs daily at {time} {offset}", { time: formatTime12h(cfg.time), offset });
    case "weekdays":
      return t("Runs weekdays at {time} {offset}", { time: formatTime12h(cfg.time), offset });
    case "weekly":
      return t("Runs every {days} at {time} {offset}", {
        days: formatDayList(cfg.daysOfWeek),
        time: formatTime12h(cfg.time),
        offset,
      });
    case "custom":
      return t("Custom schedule: {cron}", { cron: cfg.cronExpression });
  }
}
  ...
  return (
    <div className="space-y-3">
      ...
      {config.frequency === "custom" ? (
        /* Custom cron input */
        <div>
          <label className="text-xs text-muted-foreground">{t("Cron Expression")}</label>
          <input
            type="text"
          ...
          <p className="text-xs text-muted-foreground mt-1">
            {t("Standard 5-field cron (min hour dom month dow)")}
          </p>
        </div>
      ) : (
        <>
