...
const OUTPUT_MODES: {
...
}[] = [
  {
    ...
    description: t("Each run creates a tracked issue"),
    Icon: FilePlus2,
  },
  {
    value: "run_only",
    label: t("Run only"),
    description: t("Silent run, no issue created"),
    Icon: Play,
  },
];
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isCreate) {
        ...
        onOpenChange(false);
        if (scheduleOk) toast.success(t("Autopilot created"));
        else toast.error(t("Autopilot created, but schedule failed to save"));
      } else {
        ...
        onOpenChange(false);
        if (scheduleOk) toast.success(t("Autopilot updated"));
        else toast.error(t("Autopilot updated, but schedule failed to save"));
      }
    } catch {
      toast.error(isCreate ? t("Failed to create autopilot") : t("Failed to update autopilot"));
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        <DialogTitle className="sr-only">
          {isCreate ? t("New Autopilot") : t("Edit Autopilot")}
        </DialogTitle>
        ...
        <div className="flex items-center justify-between px-5 pt-3 pb-2 shrink-0 border-b">
          <div className="flex items-center gap-2 text-xs">
            ...
            <span className="text-muted-foreground">{t("A recurring AI task")}</span>
            {workspaceName && (
              <>
          ...
          <div className="flex-1 min-h-0 flex flex-col border-b lg:border-b-0 lg:border-r">
            ...
            <div className="px-6 pb-2 shrink-0 flex items-baseline gap-2">
              ...
              <span className="text-xs text-muted-foreground/80">
                {t("Read by the agent on every run")}
              </span>
            </div>
        ...
        <div className="flex items-center justify-between gap-3 px-5 py-3 border-t shrink-0 bg-background">
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground min-w-0">
            ...
            <span className="truncate">
              {t("Once saved, runs automatically until paused.")}
            </span>
          </div>
...
function OutputModeSection({
  ...
  return (
    <div>
      <SectionLabel>{t("Output mode")}</SectionLabel>
