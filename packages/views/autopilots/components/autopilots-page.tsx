...
const TEMPLATES: AutopilotTemplate[] = [
  {
    title: t("Daily news digest"),
    summary: t("Search and summarize today's news for the team"),
...
5. Post the digest as a comment on this issue and @mention all workspace members`,
  ...
  {
    title: t("PR review reminder"),
    summary: t("Flag stale pull requests that need review"),
...
5. @mention the team to remind them to review`,
  ...
  {
    title: t("Bug triage"),
    summary: t("Assess and prioritize new bug reports"),
...
5. Add a comment explaining your assessment and suggested next steps`,
  ...
  {
    title: t("Weekly progress report"),
    summary: t("Compile a weekly summary of team progress"),
...
6. Post the report as a comment on this issue`,
  ...
  {
    title: t("Dependency audit"),
    summary: t("Scan for security vulnerabilities and outdated packages"),
...
5. Post a summary report as a comment with actionable items`,
  ...
  {
    title: t("Documentation check"),
    summary: t("Review recent changes for documentation gaps"),
...
function formatRelativeDate(date: string): string {
  ...
  if (days === 1) return t("1d ago");
  if (days < 30) return t("{days}d ago", { days });
  const months = Math.floor(days / 30);
  return t("{months}mo ago", { months });
}

const STATUS_CONFIG: Record<string, { label: string; color: string; icon: typeof Zap }> = {
  active: { label: t("Active"), color: "text-emerald-500", icon: Play },
  paused: { label: t("Paused"), color: "text-amber-500", icon: Pause },
  archived: { label: t("Archived"), color: "text-muted-foreground", icon: AlertCircle },
};
  ...
  return (
    <div className="group/row flex h-11 items-center gap-2 px-5 text-sm transition-colors hover:bg-accent/40">
      ...
      <span className="w-24 shrink-0 text-center text-xs text-muted-foreground">
        {autopilot.execution_mode === "create_issue" ? t("Create Issue") : autopilot.execution_mode === "run_only" ? t("Run Only") : autopilot.execution_mode}
      </span>
    ...
    <div className="flex h-full flex-col">
      ...
      <PageHeader className="justify-between px-5">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-sm font-medium">{t("Autopilot")}</h1>
      ...
      <div className="flex-1 overflow-y-auto">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center py-16 px-5">
            ...
            <p className="text-sm text-muted-foreground">{t("No autopilots yet")}</p>
            <p className="text-xs text-muted-foreground mt-1 mb-6">
              {t("Schedule recurring tasks for your AI agents. Pick a template or start from scratch.")}
            </p>
          ...
          <>
            ...
            <div className="sticky top-0 z-[1] flex h-8 items-center gap-2 border-b bg-muted/30 px-5 text-xs font-medium text-muted-foreground">
              ...
              <span className="w-32 shrink-0">{t("Agent")}</span>
              <span className="w-24 text-center shrink-0">{t("Mode")}</span>
              <span className="w-20 text-center shrink-0">{t("Status")}</span>
              <span className="w-20 text-right shrink-0">{t("Last run")}</span>
            </div>
