  ...
  return (
    <div className="flex items-center gap-3 rounded-md border px-3 py-2">
      ...
      <AlertDialog open={confirmOpen} onOpenChange={(v) => { if (!v && !deleting) setConfirmOpen(false); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("Delete trigger")}</AlertDialogTitle>
            <AlertDialogDescription>
...
function AddTriggerDialog({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setLabel("");
      toast.success(t("Trigger added"));
    } catch {
      toast.error(t("Failed to add trigger"));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-sm">
        <DialogTitle>{t("Add Trigger")}</DialogTitle>
        <div className="space-y-4 pt-2">
          ...
          <div>
            ...
            <input
              ...
              placeholder={t("e.g. Weekday morning")}
          ...
          <div className="flex justify-end pt-1">
            <Button size="sm" onClick={handleSubmit} disabled={submitting}>
              {submitting ? t("Adding...") : t("Add trigger")}
            </Button>
          </div>
        </div>
      </DialogContent>
...
export function AutopilotDetailPage({ autopilotId }: { autopilotId: string }) {
  ...
  const handleRunNow = async () => {
    try {
      ...
      toast.success(t("Autopilot triggered"));
    } catch (e: any) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.error(t("Failed to delete autopilot"));
      setDeleting(false);
    }
  };
    ...
    <div className="flex h-full flex-col">
      ...
      <PageHeader className="justify-between px-5">
        <div className="flex items-center gap-2">
          ...
          <div className="ml-1 flex items-center gap-1.5">
            <Switch
              ...
              aria-label={autopilot.status === "active" ? t("Pause autopilot") : t("Activate autopilot")}
            />
            <span className={cn(
        ...
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" onClick={handleRunNow} disabled={autopilot.status !== "active" || triggerAutopilot.isPending}>
            ...
            {triggerAutopilot.isPending ? t("Running...") : t("Run now")}
          </Button>
        </div>
      </PageHeader>
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-4xl mx-auto p-6 space-y-8">
          {/* Properties */}
          <section className="space-y-4">
            <h2 className="text-sm font-medium text-muted-foreground uppercase tracking-wider">{t("Properties")}</h2>
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <div className="mt-1">
                  {autopilot.execution_mode === "create_issue" ? t("Create Issue") : t("Run Only")}
                </div>
              </div>
          ...
          <section className="space-y-3">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-medium text-muted-foreground uppercase tracking-wider">{t("Triggers")}</h2>
            ...
            {triggers.length === 0 ? (
              <div className="rounded-md border border-dashed p-4 text-center text-sm text-muted-foreground">
                {t("No triggers configured. Add a schedule to run automatically.")}
              </div>
            ) : (
          ...
          {/* Run History */}
          <section className="space-y-3">
            <h2 className="text-sm font-medium text-muted-foreground uppercase tracking-wider">{t("Run History")}</h2>
            {runsLoading ? (
              <div className="space-y-1">
                {Array.from({ length: 3 }).map((_, i) => (
                  <Skeleton key={i} className="h-10 w-full" />
              ...
              <div className="rounded-md border border-dashed p-4 text-center text-sm text-muted-foreground">
                {t("No runs yet. Click \"Run now\" to trigger manually.")}
              </div>
            ) : (
          ...
          <section className="space-y-3 pt-4 border-t">
            <h2 className="text-sm font-medium text-destructive uppercase tracking-wider">{t("Danger Zone")}</h2>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("Delete autopilot")}</AlertDialogTitle>
            <AlertDialogDescription>
