...
export function LoginPage({
      ...
      try {
        ...
        setError(
          err instanceof Error
            ? err.message
            : t("Failed to send code. Make sure the server is running."),
        );
      } finally {
        setLoading(false);
      }
  ...
  const handleVerify = useCallback(
    async (value: string) => {
      ...
      try {
        ...
        setError(
          err instanceof Error ? err.message : t("Invalid or expired code"),
        );
        setCode("");
        setLoading(false);
      }
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : t("Failed to resend code"),
      );
    }
  };
  ...
  const handleCliAuthorize = async () => {
    ...
    try {
      ...
      setError(t("Failed to authorize CLI. Please log in again."));
      ...
      <div className="flex min-h-svh items-center justify-center">
        <Card className="w-full max-w-sm">
          <CardHeader className="text-center">
            ...
            <CardTitle className="text-2xl">{t("Authorize CLI")}</CardTitle>
            <CardDescription>
              {t("Allow the CLI to access Multica as")}{" "}
        ...
        <Card className="w-full max-w-sm">
          ...
          <CardContent className="flex flex-col items-center gap-4">
            ...
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              >
                {cooldown > 0
                  ? t("Resend in {{seconds}}s", { seconds: String(cooldown) })
                  : t("Resend code")}
              </button>
            </div>
          </CardContent>
  ...
  return (
    <div className="flex min-h-svh items-center justify-center">
      <Card className="w-full max-w-sm">
        <CardHeader className="text-center">
          {logo && <div className="mx-auto mb-4">{logo}</div>}
          <CardTitle className="text-2xl">{t("Sign in to Multica")}</CardTitle>
          <CardDescription>
            {t("Enter your email to get a login code")}
          </CardDescription>
        </CardHeader>
        <CardContent>
