...
function AgentDropdown({
  ...
  if (!activeAgent) {
    return <span className="text-xs text-muted-foreground">{t("No agents")}</span>;
  }
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start" side="top" className="max-h-80 w-auto max-w-64">
        {mine.length > 0 && (
          <DropdownMenuGroup>
            <DropdownMenuLabel>{t("My agents")}</DropdownMenuLabel>
            {mine.map((agent) => (
              <AgentMenuItem
                key={agent.id}
                agent={agent}
...
function SessionDropdown({
  ...
  const title = activeSession?.title?.trim() || t("New chat");
    ...
    <div className="flex flex-1 flex-col items-center justify-center gap-5 px-6 py-8">
      <div className="text-center space-y-1">
        <h3 className="text-base font-semibold">
          {agentName ? t("Hi, I'm {{name}}", { name: agentName }) : t("Welcome to Multica")}
        </h3>
        <p className="text-sm text-muted-foreground">{t("Try asking")}</p>
      </div>
