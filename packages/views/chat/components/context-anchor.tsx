...
export function ContextAnchorButton() {
  ...
  const tooltipText = isDisabled
    ? t("Nothing to share with Multica on this page")
    : focusMode && candidate
      ? candidate.type === "issue"
        ? t("Multica knows you're viewing {{label}} · Click to turn off", { label: candidate.label })
        : t("Multica knows you're viewing project {{label}} · Click to turn off", { label: candidate.label })
      : t("Let Multica know what you're viewing");
      ...
      <TooltipTrigger
        render={
          <Button
            ...
            disabled={isDisabled}
            aria-label={
              focusMode ? t("Stop sharing current page") : t("Share current page with Multica")
            }
...
export function ContextAnchorCard() {
  ...
  const tooltipText =
    candidate.type === "issue"
      ? t("Multica knows you're viewing {{label}}", { label: candidate.label }) + (candidate.subtitle ? ` — ${candidate.subtitle}` : "")
      : t("Multica knows you're viewing project {{label}}", { label: candidate.label });
