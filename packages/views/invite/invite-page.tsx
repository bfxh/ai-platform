...
export function InvitePage({ invitationId, onBack }: InvitePageProps) {
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : t("Failed to accept invitation"));
    } finally {
      setAccepting(false);
    }
  };
  ...
  const handleDecline = async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : t("Failed to decline invitation"));
    } finally {
      setDeclining(false);
    }
  };
      ...
      <InviteShell onBack={onBack}>
        <Card className="w-full max-w-md">
          <CardContent className="flex flex-col items-center gap-4 py-12">
            ...
            <h2 className="text-lg font-semibold">{t("Invitation not found")}</h2>
            <p className="text-sm text-muted-foreground text-center">
              {t("This invitation may have expired, been revoked, or doesn't belong to your account.")}
            </p>
      ...
      <InviteShell onBack={onBack}>
        <Card className="w-full max-w-md">
          <CardContent className="flex flex-col items-center gap-4 py-12">
            ...
            <h2 className="text-lg font-semibold">{t("You joined {{name}}!", { name: invitation.workspace_name })}</h2>
            <p className="text-sm text-muted-foreground">{t("Redirecting to workspace...")}</p>
          </CardContent>
        </Card>
      </InviteShell>
    );
  ...
  if (done === "declined") {
    return (
      <InviteShell onBack={onBack}>
        <Card className="w-full max-w-md">
          <CardContent className="flex flex-col items-center gap-4 py-12">
            <h2 className="text-lg font-semibold">{t("Invitation declined")}</h2>
            <p className="text-sm text-muted-foreground">{t("You won't be added to this workspace.")}</p>
            <Button variant="outline" onClick={() => push(fallbackDest)}>
              {t("Go to dashboard")}
            </Button>
          </CardContent>
        ...
        <CardContent className="flex flex-col items-center gap-6 py-12">
          ...
          <div className="text-center space-y-2">
            <h2 className="text-xl font-semibold">
              {t("Join {{name}}", { name: invitation.workspace_name ?? "workspace" })}
            </h2>
            <p className="text-sm text-muted-foreground">
              {t("{{name}} invited you to join as {{role}}.", {
                name: invitation.inviter_name || invitation.inviter_email,
                role: invitation.role === "admin" ? t("an admin") : t("a member"),
              })}
            </p>
          </div>
          ...
          {isAlreadyHandled ? (
            <div className="text-sm text-muted-foreground">
              {t("This invitation has already been {{status}}.", { status: t(invitation.status) })}
            </div>
          ) : isExpired ? (
            <div className="text-sm text-muted-foreground">
            ...
            <div className="flex gap-3 w-full">
              ...
              <Button
                ...
                disabled={accepting || declining}
              >
                {accepting ? t("Joining...") : t("Accept & Join")}
              </Button>
            </div>
          )}
