...
const roleConfig: Record<MemberRole, { label: string; icon: typeof Crown; description: string }> = {
  owner: { label: t("Owner"), icon: Crown, description: "Full access, manage all settings" },
  admin: { label: t("Admin"), icon: Shield, description: "Manage members and settings" },
  member: { label: t("Member"), icon: User, description: "Create and work on issues" },
};
          ...
          <Card>
            <CardContent className="space-y-3">
              ...
              <div className="grid gap-3 sm:grid-cols-[1fr_120px_auto]">
                ...
                >
                  {inviteLoading ? "Inviting..." : t("Invite")}
                </Button>
              </div>
            </CardContent>
          </Card>
      ...
      {invitations.length > 0 && (
        <section className="space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold">{t("Pending invitations ({count})").replace("{count}", invitations.length.toString())}</h2>
          </div>
          <div className="overflow-hidden rounded-xl ring-1 ring-foreground/10">
