  ...
  return (
    <div className="space-y-8">
          ...
          <CardContent className="space-y-3">
            ...
            <div className="grid gap-3 sm:grid-cols-[1fr_120px_auto]">
              <Input
                ...
                placeholder={t("Token name (e.g. My CLI)")}
              />
              ...
              <Button onClick={handleCreateToken} disabled={tokenCreating || !tokenName.trim()}>
                {tokenCreating ? "Creating..." : t("Create")}
              </Button>
            </div>
          </CardContent>
        </Card>
      ...
      <AlertDialog open={!!revokeConfirmId} onOpenChange={(v) => { if (!v) setRevokeConfirmId(null); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("Revoke token")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t("This token will be permanently revoked and can no longer be used. This cannot be undone.")}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <Dialog open={!!newToken} onOpenChange={(v) => { if (!v) { setNewToken(null); setTokenCopied(false); } }}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {t("Copy your personal access token now. You won't be able to see it again.")}
            </DialogDescription>
          </DialogHeader>
          <div className="flex items-center gap-2">
