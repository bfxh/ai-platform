...
export function SkillsTab({
  ...
  const handleRemove = async (skillId: string) => {
    ...
    try {
      ...
      toast.error(e instanceof Error ? e.message : t("Failed to remove skill"));
    } finally {
      setRemoving(false);
    }
  };
    ...
    <div className="space-y-4">
      <div className="flex items-start justify-between gap-3">
        <p className="text-xs text-muted-foreground">
          {t("Workspace skills assigned to this agent. Local runtime skills are always available automatically.")}
        </p>
        <Button
          variant="outline"
          size="sm"
      ...
      <div className="flex items-start gap-2 rounded-md border border-info/20 bg-info/5 px-3 py-2.5">
        ...
        <p className="text-xs text-muted-foreground">
          {t("Importing creates a workspace copy that your team can edit and reuse.")}
        </p>
      </div>
      ...
      {agent.skills.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-lg border border-dashed py-12">
          ...
          <p className="mt-3 text-sm text-muted-foreground">
            {t("No skills assigned")}
          </p>
          <p className="mt-1 max-w-xs text-center text-xs text-muted-foreground">
            {t("Add workspace skills to share team knowledge with this agent.")}
          </p>
          {availableCount > 0 && (
            <Button
              onClick={() => setShowAdd(true)}
