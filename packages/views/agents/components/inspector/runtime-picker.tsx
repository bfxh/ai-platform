...
export function RuntimePicker({
  ...
  const triggerLabel = selected?.name ?? t("No runtime");
  const isOnline = selected?.status === "online";
  const triggerTitle = selected
    ? t("Runtime · {{name}} · {{status}}", { name: selected.name, status: t(isOnline ? "online" : "offline") })
    : t("Runtime · none selected");
    ...
    >
      {filtered.length === 0 ? (
        ...
        filtered.map((rt) => {
          ...
          const tooltip = [
            rt.name,
            owner ? t("owned by {{name}}", { name: owner.name }) : null,
            t(rtOnline ? "online" : "offline"),
          ]
            .filter(Boolean)
            .join(" · ");
          return (
            ...
            >
              ...
              <span
                ...
                aria-label={rtOnline ? t("online") : t("offline")}
              />
            </PickerItem>
          );
        })
