...
export function SkillAddDialog({
  ...
  const handleAdd = async (skillId: string) => {
    ...
    try {
      ...
      toast.error(e instanceof Error ? e.message : t("Failed to add skill"));
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="text-sm">{t("Add skill")}</DialogTitle>
          <DialogDescription className="text-xs">
            {t("Select a workspace skill to assign to this agent.")}
          </DialogDescription>
        </DialogHeader>
        <div className="max-h-64 space-y-1 overflow-y-auto">
          ...
          {availableSkills.length === 0 && (
            <p className="py-6 text-center text-xs text-muted-foreground">
              {t("All workspace skills are already assigned.")}
            </p>
          )}
        </div>
        <DialogFooter>
