...
import { t, tn } from "@multica/core/i18n";
import {
  Tooltip,
  TooltipContent,
  TooltipTrigger,
        ...
        <div className="flex items-center gap-2">
          ...
          {isPrivate && !isArchived && (
            <Tooltip>
              ...
              />
              <TooltipContent>
                {t("Private — only the owner can assign work")}
              </TooltipContent>
            </Tooltip>
          )}
...
function ActivityTooltip({ activity }: { activity: AgentActivity }) {
  ...
  const headerText = isPartial
    ? daysSinceCreated === 0
      ? t("today")
      : t("Created {{count}} days ago", { count: daysSinceCreated })
    : t("Last 7 days");

  let bodyText: string;
  if (totalRuns === 0) {
    bodyText = t("No activity");
  } else {
    const runsText = tn("{{count}} run", "{{count}} runs", totalRuns, { count: totalRuns });
    const failedFragment =
      totalFailed > 0
        ? t(" · {{count}} failed ({{pct}}%)", { count: totalFailed, pct: Math.round((totalFailed / totalRuns) * 100) })
        : "";
