...
export function createAuthStore(options: AuthStoreOptions) {
  ...
  return create<AuthState>((set) => ({
    ...
    initialize: async () => {
      ...
      const DEFAULT_USER_EMAIL = "default@multica.local";
      ...
      if (cookieAuth) {
        ...
        // Try to fetch the current user 鈥?if the cookie exists the server will accept it.
        try {
      ...
      try {
        ...
        // transient errors 鈥?network blips, backend rolling restarts, 5xx,
        // aborted fetches 鈥?keep the token so the next initialize() (next
