...
class LocalClaudeClient(BaseAIClient):
    ...
    def _get_default_base_url(self) -> str:
        ...
        return str(get_base_dir() / "Claude_Extracted")
    ...
    def chat(self, request: ChatRequest) -> ChatResponse:
        ...
        try:
            # 尝试使用Claude可执行文件
            claude_executable = str(Path(self.base_url) / "bin" / "claude-code-tudou")
            if not os.path.exists(claude_executable):
                claude_executable = str(Path(self.base_url) / "bin" / "claude-code-tudou.exe")
            ...
            if os.path.exists(claude_executable):
                ...
                result = subprocess.run(
                    ...
                    capture_output=True,
                    text=True,
                    cwd=os.path.join(self.base_url, "嗷嗷的", "SRC"),
                    timeout=60
                )
...
class UnifiedAI:
    ...
    def __init__(self, provider: str = None, model: str = None, **kwargs):
        ...
        self.provider = provider or get_config('ai.default_provider', 'stepfun')
        self.model = model
    ...
    def chat(self, message: str, keep_history: bool = True, 
             temperature: float = 0.7, max_tokens: int = 2000, **kwargs) -> str:
        """
        发送聊天消息
