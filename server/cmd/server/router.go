...
func NewRouterWithOptions(pool *pgxpool.Pool, hub *realtime.Hub, bus *events.Bus, analyticsClient analytics.Client, rdb *redis.Client, opts RouterOptions) chi.Router {
	...
	r.Group(func(r chi.Router) {
		...
		// --- Management panel routes (user-scoped, no workspace context) ---

		// Sub-Agent Orchestration
		r.Route("/api/sub-agent-workspaces", func(r chi.Router) {
			r.Post("/", h.CreateSubAgentWorkspace)
			r.Get("/", h.ListSubAgentWorkspaces)
			r.Route("/{subWorkspaceId}", func(r chi.Router) {
				r.Post("/spawn", h.SpawnSubAgent)
				r.Get("/spawns", h.ListSubAgentSpawns)
			})
		})
		r.Get("/api/sub-agents/{id}/status", h.GetSubAgentStatus)
		...
		r.Route("/api/snapshots", func(r chi.Router) {
			...
			r.Route("/{id}", func(r chi.Router) {
				r.Get("/", h.GetSnapshot)
				r.Delete("/", h.DeleteSnapshot)
			})
		})

		// Pending Deletions
		r.Route("/api/pending-deletions", func(r chi.Router) {
			r.Get("/", h.ListPendingDeletions)
			r.Post("/bulk-approve", h.BulkApproveDeletions)
			r.Route("/{id}", func(r chi.Router) {
				r.Post("/approve", h.ApproveDeletion)
				r.Post("/reject", h.RejectDeletion)
			})
		})

		// Resource Profiles
		r.Route("/api/resource-profiles", func(r chi.Router) {
			r.Get("/", h.ListResourceProfiles)
			r.Post("/", h.CreateResourceProfile)
		})
	})
