...
type Handler struct {
	...
	SubAgentOrchestrator   *service.SubAgentOrchestrator
	SnapshotService        *service.SnapshotService
	DeletionReviewService  *service.DeletionReviewService
	ResourceProfileService *service.ResourceProfileService
	LearnedRuleService     *service.LearnedRuleService
...
func New(queries *db.Queries, txStarter txStarter, hub *realtime.Hub, bus *events.Bus, emailService *service.EmailService, store storage.Storage, cfSigner *auth.CloudFrontSigner, analyticsClient analytics.Client, cfg Config, daemonHubs ...*daemonws.Hub) *Handler {
	...
	return &Handler{
		...
		SubAgentOrchestrator:   service.NewSubAgentOrchestrator(executor, hub, bus),
		SnapshotService:        service.NewSnapshotService(executor, hub, bus),
		DeletionReviewService:  service.NewDeletionReviewService(executor, hub, bus),
		ResourceProfileService: service.NewResourceProfileService(executor),
		LearnedRuleService:     service.NewLearnedRuleService(executor),
...
// workspaceIDFromURL returns the workspace ID from context (preferred), X-Workspace-ID header,
// workspace_id query param, or chi URL param (fallback).
