      ...
      >
        ...
        <div className="flex shrink-0 items-center justify-between border-t bg-muted/30 px-4 py-1.5 text-xs text-muted-foreground">
          <span className="tabular-nums">
            {t("Showing {{shown}} of {{total}}", { shown: filtered.length, total: logs.length })}
...
function GroupRows({
  ...
  return (
    <>
      ...
      >
        <span>···</span>
        <span>{t("collapse {{count}} repeated", { count: rest.length + 1 })}</span>
      </button>
    </>
  );
}
...
function EmptyState({
  ...
  let subtitle: string;
  if (hasFilter) {
    title = t("No matching log lines");
    subtitle = t("Try a different search or level toggle.");
  } else if (!isRunning) {
    title = t("Daemon isn't running");
    subtitle = t("Start the daemon to see logs here.");
  } else if (!hasLogs) {
    title = t("Waiting for logs…");
    subtitle = t("New entries will appear in real time.");
  } else {
    title = "";
    subtitle = "";
  }
