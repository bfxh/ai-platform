    ...
    <div className="fixed bottom-4 right-4 z-50 w-80 rounded-lg border border-border bg-background p-4 shadow-lg animate-in slide-in-from-bottom-2 fade-in duration-300">
      ...
      {state.status === "available" && (
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium">{t("New version available")}</p>
            <p className="text-xs text-muted-foreground mt-0.5">
              {t("v{{version}} is ready to download", { version: state.version })}
            </p>
            <button
      ...
      {state.status === "downloading" && (
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium">{t("Downloading update...")}</p>
