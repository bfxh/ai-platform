...
export function DaemonRuntimeCard() {
  ...
  const handleStart = useCallback(async () => {
    ...
    if (!result.success) {
      setActionLoading(false);
      toast.error(t("Failed to start daemon"), { description: result.error });
    }
  }, []);
  ...
  const performStop = useCallback(async () => {
    ...
    if (!result.success) {
      toast.error(t("Failed to stop daemon"), { description: result.error });
    }
  }, []);
  ...
  const handleRestart = useCallback(async () => {
    ...
    if (!result.success) {
      toast.error(t("Failed to restart daemon"), { description: result.error });
      return;
    }
    ...
    toast.success(t("Restarting daemon"), {
      description: t("Runtimes will be back online in a few seconds."),
    });
  }, []);
        ...
        <div className="flex items-start gap-3">
          ...
          <DialogHeader className="flex-1 gap-1">
            <DialogTitle className="text-sm font-semibold">
              {t("Stop daemon with {{count}} active task(s)?", { count: affectedCount })}
            </DialogTitle>
            <DialogDescription className="text-xs leading-relaxed">
              {t("{{count}} task(s) is/are currently running on this device. Stopping now will interrupt it/them — affected tasks get marked failed once the timeout hits. The daemon won't auto-restart.", { count: affectedCount })}
            </DialogDescription>
          </DialogHeader>
        </div>
        <DialogFooter>
