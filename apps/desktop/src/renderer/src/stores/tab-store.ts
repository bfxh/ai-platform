...
function defaultTabFor(slug: string): Tab {
  ...
  return makeTab(path, t("Issues"), resolveRouteIcon(path));
}
      ...
      switchWorkspace(slug, openPath) {
        ...
        if (!existing) {
          ...
          const seedPath =
            desiredPath && sanitizeTabPath(desiredPath) === desiredPath
              ? desiredPath
              : defaultPathFor(slug);
          const tab = makeTab(seedPath, t("Issues"), resolveRouteIcon(seedPath));
          set({
        ...
        if (desiredPath) {
          ...
          if (clean) {
            ...
            const tab = makeTab(clean, t("Issues"), resolveRouteIcon(clean));
            set({
              activeWorkspaceSlug: slug,
              byWorkspace: {
                ...byWorkspace,
